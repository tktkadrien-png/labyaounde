  ...
  <div>
    <h3 className="text-lg font-bold text-white mb-5 relative">
      <span className="relative">
        {title}
        <span className="absolute -bottom-2 left-0 w-12 h-0.5 bg-cyan-400"></span>
      </span>
    </h3>
    <ul className="space-y-3 mt-6">
      {links.map((link, index) => (
        <li key={index}>
          <Link
            href={link.href}
            className="text-sm text-white/80 hover:text-cyan-300 transition-colors duration-200 flex items-center gap-2 group"
          >
    ...
    <>
      <footer className="relative overflow-hidden" style={{ fontFamily: 'Roboto, sans-serif' }}>
        ...
        <div className="absolute inset-0">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-b from-[#0B3D5F]/95 via-[#0B3D5F]/90 to-[#063251]/95"></div>
        </div>
              ...
              <div className="flex flex-col lg:flex-row justify-between items-center gap-8">
                ...
                <div className="flex flex-wrap justify-center gap-4">
                  <a
                    ...
                    className="bg-cyan-500 hover:bg-cyan-400 text-white rounded-lg px-6 py-3 font-semibold flex items-center gap-3 transition-all duration-300 shadow-lg hover:shadow-cyan-500/30 hover:scale-105"
                  >
                    <MapPin size={20} />
                ...
                <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
                  ...
                  <div className="space-y-4">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-start gap-3 text-white/80 hover:text-cyan-300 transition-colors group"
                    >
                    ...
                    <a
                      ...
                      className="flex items-center gap-3 text-white/80 hover:text-cyan-300 transition-colors"
                    >
                        ...
                        <ul className="space-y-3 pt-2 pb-4">
                          {col.links.map((link, index) => (
                            <li key={index}>
                              <Link
                                href={link.href}
                                className="text-sm text-white/70 hover:text-cyan-300 transition-colors"
                              >
                                {link.text}
                              </Link>
                            </li>
            ...
            <div className="max-w-[1200px] mx-auto px-6 py-6">
              <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                <p className="text-sm text-white/60 text-center md:text-left">
                  © 2025 <span className="text-cyan-400 font-semibold">LABYAOUNDE</span>. Tous droits réservés
                </p>
      ...
      {showBackToTop && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-gradient-to-r from-cyan-500 to-cyan-600 text-white flex items-center justify-center shadow-lg hover:shadow-cyan-500/40 hover:scale-110 transition-all duration-300"
        >
          <ArrowUp size={24} />
        </button>
      )}
