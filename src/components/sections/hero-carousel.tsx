...
export default function HeroCarousel() {
  ...
  return (
    <section className="relative w-full" aria-label="Hero Carousel">
      ...
      >
        <CarouselContent>
          {slides.map((slide, index) => (
            ...
            >
              <div className="relative w-full min-h-[600px] h-screen max-h-[880px] overflow-hidden">
                <Image
                  src={slide.imageSrc}
                  alt={slide.title}
                  fill
                ...
                <div className="absolute inset-0 flex items-center">
                  <div className="container">
                    <div className="w-full md:w-3/5 lg:w-1/2 xl:w-2/5">
                      <div className="w-16 h-1 bg-cyan-400 mb-6 rounded-full" />

                      <h1 className="font-bold text-3xl md:text-4xl lg:text-5xl leading-tight text-white drop-shadow-lg">
                        {slide.title}
                      </h1>

                      {slide.subtitle && (
                        <p className="mt-5 text-base md:text-lg text-white/90 leading-relaxed max-w-md drop-shadow-md">
                          {slide.subtitle}
                        </p>
                      )}
                      ...
                      <Link href={slide.buttonLink} passHref>
                        <Button
                          asChild
                          className="mt-8 bg-cyan-500 hover:bg-cyan-600 text-white border-none rounded-lg px-8 py-6 text-base font-semibold shadow-lg hover:shadow-xl transition-all duration-300 group"
                        >
                          <a>
                            {slide.buttonText}
        ...
        <CarouselPrevious className="absolute left-6 top-1/2 -translate-y-1/2 z-10 h-14 w-14 text-white bg-white/10 backdrop-blur-sm border border-white/20 hover:bg-white/20 hover:border-white/40 transition-all duration-300 rounded-full" />
        <CarouselNext className="absolute right-6 top-1/2 -translate-y-1/2 z-10 h-14 w-14 text-white bg-white/10 backdrop-blur-sm border border-white/20 hover:bg-white/20 hover:border-white/40 transition-all duration-300 rounded-full" />
      </Carousel>

      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10">
        <div className="flex items-center gap-3 bg-black/30 backdrop-blur-md rounded-full px-4 py-3">
          {slides.map((_, index) => (
            <button
              key={index}
      ...
      <div className="absolute bottom-8 right-8 z-10 hidden md:flex items-center gap-2 text-white/80 text-sm font-medium">
