              ...
              {currentContent.sections.map((section, index) => {
                ...
                return (
                  <Link
                    key={index}
                    href={section.link}
                    className="bg-white rounded-2xl p-8 shadow-lg border-2 border-gray-100 hover:border-[#2916F5] hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 group"
                  >
                    <div className="flex items-start gap-4">
                      <div className="w-14 h-14 bg-gradient-to-br from-[#2916F5] to-[#157DEC] rounded-xl flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform">
                        <Icon className="w-7 h-7 text-white" />
                      </div>
                      <div className="flex-1">
                        <h3 className="text-2xl font-bold text-gray-900 mb-3 group-hover:text-[#2916F5] transition-colors">
                          {section.title}
                        </h3>
                        <p className="text-gray-600 leading-relaxed mb-4">{section.description}</p>
                        <div className="flex items-center gap-2 text-[#2916F5] font-semibold">
