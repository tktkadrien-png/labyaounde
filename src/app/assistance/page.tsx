            ...
            <div className="grid md:grid-cols-3 gap-8">
              {currentContent.sections.map((section, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="bg-white rounded-2xl p-8 shadow-lg border-2 border-gray-100 hover:border-[#2916F5] hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
                  >
                    ...
                    <ul className="space-y-2 mb-6">
                      {section.details.map((detail, i) => (
                        <li key={i} className="text-sm text-gray-700 flex items-center gap-2">
                          <div className="w-1.5 h-1.5 bg-[#2916F5] rounded-full"></div>
                          {detail}
                        </li>
                      ))}
                    </ul>
        ...
        <section className="py-16 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid md:grid-cols-2 gap-8">
              {/* Schedule */}
              <div className="bg-gradient-to-br from-[#1589FF]/10 to-[#1589FF]/10 rounded-2xl p-8 border-2 border-[#1589FF]/30">
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-12 h-12 bg-[#2916F5] rounded-xl flex items-center justify-center">
                ...
                <div className="space-y-4">
                  {currentContent.schedule.items.map((item, index) => (
                    <div key={index} className="flex justify-between items-center bg-white p-4 rounded-xl">
                      ...
                      <span className="text-[#2916F5] font-bold">{item.hours}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-8 border-2 border-purple-200">
                ...
                <a
                  ...
                  className="inline-flex items-center gap-2 bg-[#2916F5] text-white px-6 py-3 rounded-xl font-semibold hover:bg-[#157DEC] transition-all shadow-lg"
                >
